...
//    private List<DroneVideoListener>        image_listeners   = new LinkedList<DroneVideoListener>();
    private List<IDroneVideoListenerMOCK>        image_listeners   = new LinkedList<IDroneVideoListenerMOCK>();
    private List<NavDataListener>           navdata_listeners = new LinkedList<NavDataListener>();

    public ARDroneMOCK() throws UnknownHostException
    {
//        this(InetAddress.getByAddress(DEFAULT_DRONE_IP));
    }

    public ARDroneMOCK(InetAddress drone_addr)
    {
//        this.drone_addr = drone_addr;
    }

    public void addImageListener(IDroneVideoListenerMOCK l)
    {
...
//        cmd_queue.add(new FlatTrimCommand());
    ...
    public void videoFrameReceived(int startX, int startY, int w, int h, IplImage im, /*int[] rgbArray,*/ int offset, int scansize)
    {
        synchronized(image_listeners)
        {
            for(IDroneVideoListenerMOCK l : image_listeners)
                l.frameReceived(startX, startY, w, h, im,/*rgbArray,*/ offset, scansize);
        }
    }
